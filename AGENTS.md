...
- `serviceId` is this repo's name with the hyphen removed — `neo-08` → `neo08`.
...
Pushing to `main` publishes `ghcr.io/neueda-learning/neo-08-{backend,frontend}` and **this repo
...
- **The front end's path prefix is baked at build time.** `APP_BASE_PATH=/neo-08` is a
